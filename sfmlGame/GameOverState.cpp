...
#include <fstream>
	...
	void GameOverState::Init()
	{
		...
		readFile.close();
		ofstream writeFile("Resources/res/HighScore.txt");
		...
		_gameOverTitle.setPosition((_data->window.getSize().x / 2) - (_gameOverBody.getGlobalBounds().width / 2),
			_gameOverBody.getPosition().y-(_gameOverTitle.getGlobalBounds().height*1.2));
		_retryButton.setPosition((_data->window.getSize().x / 2) - (_gameOverBody.getGlobalBounds().width / 2),
		...
		_scoreText.setPosition(_data->window.getSize().x / 10 * 725, _data->window.getSize().y/2.15);
		_scoreText.setFont(_data->assets.GetFont("Batty Font"));
		_scoreText.setString(to_string(_score));
		_scoreText.setCharacterSize(56);
		_scoreText.setFillColor(sf::Color::White);
		...
		_highScoreText.setFont(_data->assets.GetFont("Batty Font"));
		_highScoreText.setString(to_string(_score));
		_highScoreText.setCharacterSize(56);
		_highScoreText.setFillColor(sf::Color::White);
		_highScoreText.setOrigin(_highScoreText.getGlobalBounds().width / 2, _highScoreText.getGlobalBounds().height / 2);
		_highScoreText.setPosition(_data->window.getSize().x / 10 * 725, _data->window.getSize().y / 1.78);
